...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    @Override
    public EmployeeDto updateEmployee(Long employeeId, EmployeeDto UpdateEemployee) {
        Employee employee = employeeRepository.findById(employeeId).orElseThrow(
                () -> new ResourceNotFoundException("Employee with id " + employeeId + " not found")
        );
        employee.setFirstName(UpdateEemployee.getFirstName());
        employee.setLastName(UpdateEemployee.getLastName());
        employee.setEmail(UpdateEemployee.getEmail());
